...
nome_avaliador = avaliadores_unicos[0] if avaliadores_unicos else "Não informado"

st.sidebar.markdown(
    f"""
    <div style="background-color: #E8F1FB; border-left: 5px solid #1565C0; padding: 10px 12px; margin: 8px 0 14px;">
        <div style="font-size: 0.78rem; color: #4A5568;">AVALIADOR</div>
        <div style="font-size: 1.05rem; font-weight: 700; color: #123B66;">{nome_avaliador}</div>
    </div>
    """,
    unsafe_allow_html=True
)
...
# -------------------------------------------------------------
    ...
    AvalEquipe = st.checkbox("Exibir avaliação geral / da equipe")
